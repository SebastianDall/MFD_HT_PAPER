...
# define paths
dir_in=('/path/to/mfd_ht_paper')
seq_in=('/path/to/mfd_ht_paper/sequences_trim')
dir_out_f=('forward')
dir_out_r=('reverse')
hmms=('/path/to/hmms')
udb=('/path/to/sintax_database.udb')
...
while read -r line; do
  ...
  seqkit grep -f $new_dir/tmp/forward_IDs_unique.txt $input'R1.fastq.gz' -o $new_dir/forward/'arc_bac_'$new_name'_forward.fq'
  seqkit grep -f $new_dir/tmp/forward_euk_IDs.txt $input'R1.fastq.gz' -o $new_dir/forward/'euk_'$new_name'_forward.fq'
  seqkit grep -f $new_dir/tmp/reverse_IDs_unique.txt $input'R2.fastq.gz' -o $new_dir/reverse/'arc_bac_'$new_name'_reverse.fq'
