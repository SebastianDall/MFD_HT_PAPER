...
```{r}
plot.ordinate <- amp_subset_samples(dat_types, minreads = 10000) %>%
...
plot.ordinate.insert <- plot.ordinate + 
  annotation_custom(ggplotGrob(plot.insert), xmin = -0.15, xmax = 0.35, ymin = 0.0, ymax = 0.5) +
  geom_rect(aes(xmin = -0.15, xmax = 0.35, ymin = 0.0, ymax = 0.5), color='black', linetype='dashed', alpha=0) +
  geom_rect(aes(xmin = -0.25, xmax = -0.175, ymin = -0.03, ymax = 0.02), color = "black", alpha = 0) +
    geom_segment(aes(x = -0.25, y = 0.02, xend = -0.15, yend = 0.5), color = "black", linetype = "dashed") +
      geom_segment(aes(x = -0.175, y = -0.03, xend = 0.35, yend = 0.0), color = "black", linetype = "dashed")
...
p.h2 <- dat_types_sub10000 %>%
  amp_subset_samples(Kit %in% c("PowerSoil Pro HT", "ZymoMagbead HT", "FastSpin HT")) %>%
  amp_heatmap(facet_by = NULL,
            group_by = "Kit",
            tax_aggregate = "Phylum",
            #tax_add = c("Phylum"),
            tax_show = 20,
  ...
  labs(x = "",
       y = "",
       title = "Heatmap aggregated at the phylum level - HT extraction kits") +
  theme_bw(base_size = 10) +
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 90, face = "bold", vjust = 0.5, size = 10),
        axis.text.y = element_text(face = "bold", size = 10),
...
p.h3 <- dat_types_sub10000 %>%
  amp_subset_samples(Kit %in% c("PowerSoil Pro HT", "ZymoMagbead HT", "FastSpin HT")) %>%
  amp_subset_taxa(tax_vector = c("p__Actinobacteriota", "p__Firmicutes")) %>%
  amp_heatmap(facet_by = NULL,
            group_by = "Kit",
            tax_aggregate = "Genus",
            tax_add = c("Phylum"),
  ...
  labs(x = "",
       y = "",
       title = "Heatmap aggregated at the genus level for gram positive phyla\n - HT extraction kits") +
  theme_bw(base_size = 10) +
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 90, face = "bold", vjust = 0.5, size = 10),
        axis.text.y = element_text(face = "bold", size = 10),
...
plot.sig1 <- res$signif_plotdata %>%
  ...
  scale_fill_manual(values = colors) +
  theme_bw(base_size = 10) +
  theme(axis.text.x = element_text(angle = 90, face = "bold", size = 10, vjust = 0.5),
        plot.title = element_text(hjust = 0.5, face = "bold", size = 10),
        axis.text.y = element_text(face = "bold", size = 10),
...
meta <- dat_types_sub10000_ra$metadata %>% select(SeqID,Kit,Soil_type) %>%
...
```{r}
ggplot(filter(phylum,Phylum %in% res_diff$Phylum),
       aes(x = Kit,y = pct)) +
