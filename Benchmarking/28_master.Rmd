...
```{r}
tax_data_rel_0.1 <- dat_types_sub10000_ra$abund %>%
  t() %>%
  as.data.frame() %>%
  abundance_filter(abundance_threshold = 0.1, verbose = T)
...
bray_with_metadata <- metadata %>%
  ...
  left_join(rename(
    select(metadata, SeqID, Kit, Soil_type),
    comparison = SeqID,
...
mean_sd_table_wide <- mean_sd_table_long %>%
  ...
  # mutate(
  #   long_lati = str_replace(long_lati, ",", "\n"),
  #   long_lati = if_else(Kit == "FastSpin", str_replace(long_lati, "\n.*", ""), long_lati),
  #   long_lati = if_else(Kit == "FastSpin HT", str_replace(long_lati, ".*\n", ""), long_lati),
  #   long_lati = if_else(str_detect(long_lati, "\n"), "", long_lati)
  # ) %>%
  group_by(Soil_type) %>%
  mutate_at(vars(Soil_type, Soil_properties), funs(replace(., duplicated(.), ""))) # %>%
# mutate(bray = str_replace(bray, " .*", ""))
