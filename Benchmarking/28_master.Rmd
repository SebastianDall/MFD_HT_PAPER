...
## Alpha diversity + failed lib tabke
...
alpha_table <- alpha_div_subs10000_ra_summarise %>%
  ...
  table_cell_font(row = filter(whereNA, variable == "failed_lib", value > 0)$rownumber, column = 3, face = "bold", color = "black", hjust = 0)
...
# guide(fill = guide_legend(label.vjust = -3))


plot.ordinate + guides(fill = guide_legend(label.vjust = -3))
...
# Plotting
```{r}
p <- ggarrange(alpha_table, plot.ordinate, ncol = 2, common.legend = T, legend = "bottom", heights = c(0.5, 1))

ggsave(here("Benchmarking", "figures", "ordination_CA_table.png"), device = "png", height = 11, width = 13, plot = p)
```
