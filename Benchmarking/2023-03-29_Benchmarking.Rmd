...
tmp <- t(dat.types.sub10000.amp.rel_0.1$abund)
...
for (i in 1:length(soil)) {
 ...
 permanova <- adonis2(community_0.1 ~ kit, type = "bray", data = meta, permutations = 9999) %>%
   as.data.frame() %>%
   rownames_to_column("Variable") %>%
...
set.seed(123)
permanova.stratified <- adonis2(community_0.1 ~ kit*soil_type, type = "bray", data = meta, permutations = 9999)
...
plot.ordinate <- dat.types.sub10000.amp %>%
  ...
  coord_cartesian(xlim = c(-0.5, 0.5), ylim = c(-0.4, 0.6)) +
  articlethemex0 + 
...
plot.insert <- plot.ordinate +
  coord_cartesian(ylim = c(0.5425, 0.56), xlim = c(0.185, 0.21)) +
...
plot.ordinate.insert <- plot.ordinate + 
  ...
  geom_segment(aes(x = 0.21, y = 0.56, xend = -0.1, yend = 0.625), color = "black", linetype = "dashed") +
  #geom_segment(aes(x = 0.19, y = 0.54, xend = -0.3, yend = 0.25), color = "black", linetype = "dashed") +
  geom_segment(aes(x = 0.21, y = 0.5425, xend = -0.1, yend = 0.25), color = "black", linetype = "dashed") +
  annotation_custom(ggplotGrob(plot.insert), xmin = -0.5, xmax = -0.1, ymin = 0.25, ymax = 0.625) +
  geom_rect(aes(xmin = 0.185, xmax = 0.21, ymin = 0.5425, ymax = 0.56), color='black', alpha = 0) #+
...
```{r ordination}
soil <- unique(dat.types.sub10000.amp$metadata$soil_type) %>% sort(.) %>% as.factor(.)
...
plot_ord1 <- dat.types.sub10000.amp %>%
  ...
  labs(title = paste0(soil[1], ": ", "PCA of microbial diversity"),
       tag = "A",
       subtitle = paste0("**PERMANOVA:**", "<br>Extraction kit: Variance explained = ", permanova.list[[1]][1,2], " %,", " p-value ", permanova.list[[1]][1,3])) +
...
plot_ord2 <- dat.types.sub10000.amp %>%
  ...
  labs(title = paste0(soil[2], ": ", "PCA of microbial diversity"),
       tag = "B",
       subtitle = paste0("**PERMANOVA:**", "<br>Extraction kit: Variance explained = ", permanova.list[[2]][1,2], " %,", " p-value ", permanova.list[[2]][1,3])) +
  coord_cartesian(xlim = c(-0.5, 0.5), ylim = c(-0.5, 0.5)) +
  articlethemex0 +
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 0, hjust = 0, vjust = 0),
...
plot_ord3 <- dat.types.sub10000.amp %>%
  ...
  labs(title = paste0(soil[3], ": ", "PCA of microbial diversity"),
       tag = "C",
       subtitle = paste0("**PERMANOVA:**", "<br>Extraction kit: Variance explained = ", permanova.list[[3]][1,2], " %,", " p-value ", permanova.list[[3]][1,3])) +
  articlethemex0 +
  coord_cartesian(xlim = c(-0.25, 0.75), ylim = c(-0.5, 0.5)) +
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 0, hjust = 0, vjust = 0),
        plot.subtitle = ggtext::element_markdown())
...
plot_ord4 <- dat.types.sub10000.amp %>%
  ...
  labs(title = paste0(soil[4], ": ", "PCA of microbial diversity"),
       tag = "D",
       subtitle = paste0("**PERMANOVA:**", "<br>Extraction kit: Variance explained = ", permanova.list[[4]][1,2], " %,", " p-value ", permanova.list[[4]][1,3])) +
  coord_cartesian(xlim = c(-0.5, 0.5), ylim = c(-0.5, 0.5)) +
  articlethemex0 +
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 0, hjust = 0, vjust = 0),
...
plot_ord5 <- dat.types.sub10000.amp %>%
  ...
  labs(title = paste0(soil[5], ": ", "PCA of microbial diversity"),
       tag = "E",
       subtitle = paste0("**PERMANOVA:**", "<br>Extraction kit: Variance explained = ", permanova.list[[5]][1,2], " %,", " p-value ", permanova.list[[5]][1,3])) +
  coord_cartesian(xlim = c(-0.5, 0.5), ylim = c(-0.5, 0.5)) +
  articlethemex0 +
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 0, hjust = 0, vjust = 0),
...
plot.ord <- ggarrange(plot_ord1, plot_ord2, plot_ord3, plot_ord4, plot_ord5, plot_ord_leg, nrow = 2, ncol = 3)
...
ggsave(here("Benchmarking", "figures", "PCA_faceted_benchmarking.png"), plot = plot.ord, device = "png", height = 12, width = 19)
```
