...
extraction_benchmark_v2 <- read_excel(here("Benchmarking", "data", "2022-10-13_metadata_benchmarking_exp.xlsx")) %>%
    mutate(benchmark = "v2") %>%
    clean_names()
...
# soil_lvls <- c("Blank", "Beach sand", "Sand", "Sand-clay", "Clay", "Organic", "Activated sludge")
soil_lvls <- c("Activated sludge", "Beach sand", "Clay", "Organic", "Sand", "Sand-clay", "Blank") # tbnj
```
...
df_factored_v2 <- extraction_benchmark_v2 %>%
    filter(!is.na(elution_placement)) %>%
    mutate(soil_type_fct = factor(soil_type, levels = soil_lvls)) %>%
    mutate(
        kit = str_replace(kit, "ZymoMagbead", "ZymoMagbead HT")
    ) %>%
...
gg_280 <- createGGMetricsPlotForCombined(df_factored_long, metric_filter = "260/280 ratio") +
    ...
    annotate(
        geom = "segment", x = 2, xend = 2, y = 1.3, yend = 1.7, size = 1,
