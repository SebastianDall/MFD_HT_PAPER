...
meta <- meta %>% select(SeqID, Soil_type, RPM, Time)

# Extract the OTU matrix.
otu <- amp_export_long(ampvis.data, metadata_vars = c("SeqID", "Soil_type", "RPM", "Time"), tax_levels = "OTU") %>%
...
phylum <- amp_export_long(ampvis.data,metadata_vars = c("SeqID", "Soil_type", "RPM", "Time"),tax_levels = "Phylum") %>%
  .[,pct := count/sum(count),by = SeqID]# %>%
  #.[Phylum != ""]
