...
writexl::write_xlsx(final_table, here("Bead_beating", "figures", "beadbeating_input_scale.xlsx"))
```
...
for (i in 1:length(soil)) {
 ...
 permanova <- adonis2(community_0.1 ~ sample_target, type = "bray", data = meta, permutations = 9999) %>%
   as.data.frame() %>%
   rownames_to_column("Variable") %>%
...
set.seed(123)
permanova.target <- adonis2(community_0.1 ~ sample_target*soil_type, type = "bray", data = meta, permutations = 9999)
...
plot.ordinate <- dat.types.sub3000.amp %>%
  ...
  labs(title = "All samples: PCA of microbial diversity",
       ...
       subtitle = paste0("**PERMANOVA:**", 
                         ...
                         "<br>Input amount: Variance explained = ","0.7 %,", " p-value = 0.137",
                         "<br>Intercation: Variance explained = ","2.0 %,", " p-value = 0.246")) +
...
ggsave(here("Bead_beating", "figures", "PCA_faceted_scale.png"), plot = plot.ord, device = "png", height = 12, width = 19)
```
...
p.diff <- ggarrange(plotlist = plot_list2, nrow = 2, ncol = ceiling(length(plot_list) / 2), labels = toupper(letters[1:5]), font.label = list(size = 18, face = "bold"))

p.diff

ggsave(here("Bead_beating", "figures", "differential_abundance_faceted_scale.png"), plot = p.diff, device = "png", height = 12, width = 19)
```
...
ggsave(here("Bead_beating", "figures", "combined_plot_scale.png"), plot = gg_arranged, device = "png", height = 12, width = 20)
```
