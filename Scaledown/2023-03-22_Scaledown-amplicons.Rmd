...
plot.heatmap_genus <- ampvis.sub_genus %>%
  amp_heatmap(group_by = "lib_volume", 
              ...
              plot_legendbreaks = c(0.1, 1, 3, 10),
...
for (i in 1:length(soil)) {
 ...
 permanova <- adonis2(community_0.1 ~ lib_volume, type = "bray", data = meta, permutations = 9999) %>%
   as.data.frame() %>%
   rownames_to_column("Variable") %>%
...
set.seed(123)
permanova.stratified <- adonis2(community_0.1 ~ lib_volume*soil_type, type = "bray", data = meta, permutations = 9999)
...
plot.ordinate <- dat.types.sub3000.amp %>%
  ...
  labs(title = "All samples: PCA of microbial diversity",
       ...
       subtitle = paste0("**PERMANOVA:**", 
                         ...
                         "<br>Interaction: Variance explained = ","2.1 %,", " p-value = 0.08")) +
...
plot_ord3 <- dat.types.sub3000.amp %>%
  ...
  amp_ordinate(
    ...
    species_plot = T) +
  ggforce::geom_mark_ellipse(aes(fill = soil_type, group = lib_volume), alpha = 0.5, color = "black") +
  scale_fill_manual(values = c(colors[4], 5)) +
  scale_shape_manual(values = c(16,21), labels = c("2 x 25 µL", "2 x 5 µL")) +
...
plot_ord4 <- dat.types.sub3000.amp %>%
  ...
  amp_ordinate(
    ...
    species_plot = T) +
  ggforce::geom_mark_ellipse(aes(fill = soil_type, group = lib_volume), alpha = 0.5, color = "black") +
  scale_fill_manual(values = c(colors[5], 5)) +
  scale_shape_manual(values = c(16,21), labels = c("2 x 25 µL", "2 x 5 µL")) +
...
plot_ord5 <- dat.types.sub3000.amp %>%
  ...
  amp_ordinate(
    ...
    species_plot = T) +
  ggforce::geom_mark_ellipse(aes(fill = soil_type, group = lib_volume), alpha = 0.5, color = "black") +
  scale_fill_manual(values = c(colors[2], 5)) +
  scale_shape_manual(values = c(16,21), labels = c("2 x 25 µL", "2 x 5 µL")) +
...
plot <- ggarrange(plot_diff, plot_ord_final, ncol = 1, nrow = 2)

plot

leg.heat <- cowplot::get_legend(plot.heatmap_genus)
leg.ord <- cowplot::get_legend(plot_ord3 + theme(legend.position = "right"))
leg.diff <- cowplot::get_legend(plot_list[[3]] + theme(legend.position = "right"))
