...
amp_rarecurve <- dat.types.sub3000.amp %>%
...
#tmp <- dat.types.sub3000.amp.ra 

#bray_distance2 <- as.matrix(vegan::vegdist(t(tmp$abund))) %>%
#    as.data.frame() %>%
#    rownames_to_column("seq_id")

#set.seed(123)
#tmp2 <- dat.types.sub3000.amp %>%
#  filter_otus(0.1)

#tmp2 <- tmp2 %>%
#  amp_subset_samples(., rarefy = min(colSums(tmp2$abund)))

#bray_distance3 <- as.matrix(vegan::vegdist(t(tmp2$abund))) %>%
#    as.data.frame() %>%
#    rownames_to_column("seq_id")
  
metadata <- dat.types.sub3000.amp.ra.rel_0.1$metadata %>%
...
lab.metadata.amplicon <- metadata.amplicon %>%
...
rarefied_alphadiversity <- dat.types.sub3000.amp.ra %>%
  ...
  select(seq_id, soil_type, lib_volume, observed_asv, shannon) %>%
  left_join(tab_reads)

combine_alpha_beta_with_metadata <- rarefied_alphadiversity %>%
  # add library_concentration
  right_join(lab.metadata.amplicon) %>%
...
names <- tibble(
  ...
  new_names = c("Soil type", "Protocol", "Libraries", "Library conc. [ng/µL]", "Read Count", "Observed ASVs", "Shannon Diversity", "Bray-Curtis Dissimilarity", "BC Dissimilarity\nBetween Protocols"))
...
ggsave(here("Scaledown", "figures", "table_amplicons.png"), plot = alpha_table, device = "png", height = 12, width = 19)


writexl::write_xlsx(final_table, here("Scaledown", "figures", "scaledown_amplicons_table.xlsx"))
```

## Statistical tests - ANVOA and Mann-Whitney
```{r}
# SHANNON
alpha.all <- rarefied_alphadiversity %>%
  left_join(lab.metadata.amplicon) %>%
  arrange(shannon) %>%
...
TukeyHSD(aov(aov.bray))$lib_volume
...
textmap.amplicon_genus <- dat.types.sub3000.amp %>%
...
ampvis.sub_genus <- amp_load(otutable = textmap.amplicon_genus,
                             metadata = lab.metadata.amplicon) %>%
  amp_subset_taxa(tax_vector = c("Unclassified"), normalise = FALSE, remove = TRUE)

ampvis.sub_genus_rel <- amp_load(otutable = textmap.amplicon_genus,
                             metadata = lab.metadata.amplicon) %>%
...
#              normalise = FALSE,
#              tax_show = 25,
#              #tax_show = rownames(textmap.sub_genus), 
...
#              plot_legendbreaks = c(0.001, 0.01, 0.1, 1, 10),
#              plot_colorscale = "log10") +
#  scale_x_discrete(limits = c("Small-scale", "Full-scale"), labels = c("2 x 5 µL", "2 x 25 µL")) +
#  #scale_y_discrete(labels = rev(rownames(textmap.sub_genus))) +
...
dat_types_filtered_genus <- ampvis.sub_genus_rel
...
heatmap.genus_data <- textmap.sub_genus %>%
  rownames_to_column(var = "Genus") %>%
  group_by(Genus) %>%
  pivot_longer(!Genus, names_to = "protocol", values_to = "abund") %>%
  ungroup() %>%
  separate(protocol, into = c("lib_volume", "soil_type"), sep =  " ") %>%
  mutate(across(soil_type, ~str_replace(., "Beach", "Beach Sand"))) %>%
  left_join(ProtocolDifferentialAbundance_genus %>% select(soil_type, Genus, Bias, pval_signif, fc_signif)) %>%
  mutate(across(Genus, ~str_replace(., "Burkholderia-Caballeronia-Paraburkholderia", "BCP"))) %>%
...
ggplot(aes(x = lib_volume, y = Genus)) + 
  ...
  articletheme +
  scale_fill_gradient(low = "white", high = colors[3], trans = "log10", breaks = c(0.01, 0.1, 1, 10), na.value = "white") +
  scale_x_discrete(limits = c("Small-scale", "Full-scale"), labels = c("2 x 5 µL", "2 x 25 µL")) +
...
ggsave(here("Scaledown", "figures", "heatmap_amplicons.png"), plot = plot.heatmap_genus, device = "png", height = 12, width = 19)
```
...
plot.ordinate <- dat.types.sub3000.amp %>%
  ...
  labs(title = "All samples: PCA of microbial diversity",
       ...
       subtitle = paste0("**PERMANOVA:**", 
                         "<br>Library volume: Variance explained = ","0.9 %,", " p-value = 0.03",
                         "<br>Soil type: Variance explained = ","90.6 %,", " p-value < 0.001",
                         "<br>Interaction: Variance explained = ","2.1 %,", " p-value = 0.09")) +
...
ggsave(here("Scaledown", "figures", "PCA_faceted_amplicons.png"), plot = plot.ord, device = "png", height = 12, width = 19)
ggsave(here("Scaledown", "figures", "pre_S3Fig.tif"), plot = plot.ord, device = "tiff", height = 12, width = 19, dpi = 600)
```
...
ggsave(here("Scaledown", "figures", "differential_abundance_faceted_amplicons.png"), plot = p.diff, device = "png", height = 12, width = 19)
ggsave(here("Scaledown", "figures", "pre_S5Fig.tif"), plot = p.diff, device = "tiff", height = 12, width = 19, dpi = 600)
```
...
ggsave(here("Scaledown", "figures", "combined_plot_amplicons.png"), plot = gg_arranged, device = "png", height = 12, width = 20)
ggsave(here("Scaledown", "figures", "pre_Fig3.tif"), plot = gg_arranged, device = "tiff", height = 12, width = 20, dpi = 600)
```
