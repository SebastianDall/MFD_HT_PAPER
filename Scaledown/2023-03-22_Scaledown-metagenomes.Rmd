...
ampvis.sub_genus <- amp_load(otutable = heatmap.metagenome_genus,
...
plot.heatmap_genus <- ampvis.sub_genus %>%
  ...
  labs(x = "", y = "", title = "Heatmap aggregated at the genus level", tag = "A") +
  articletheme +
...
for (i in 1:length(soil)) {
  ...
  community <- dat.types.sub3000.meta.rel_0.1 %>%
...
meta <- dat.types.sub3000.meta.rel_0.1[["metadata"]]
...
ProtocolDifferentialAbundance <- combine_otu_w_metadata(metadata = dat.types.sub3000.meta$metadata, otu = otu) %>%
  ...
  mutate(Bias = if_else(log2FoldChange < 0 & pval_signif, "More abundant with 1 x 5 µL", 
                        if_else(pval_signif & log2FoldChange > 0, "More abundant with 1 x 50 µL", "Equally detected")),
...
plot_diff <- plot_list[[3]] + 
  labs(title = "Organic: Differential abundance of OTUs", subtitle =  "OTUs with relative abundance above 0.1 %", tag = "B") + 
...
leg.comb <- cowplot::plot_grid(leg.heat, leg.diff, leg.ord, ncol = 1, align = "v", nrow = 3, rel_heights = c(1, 1, 1)) + theme(plot.margin = unit(c(0, 1, 0, 0), "cm"))
