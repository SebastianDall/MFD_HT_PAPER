...
# Rename genera
genus.text.meta <- textmap.metagenome_genus %>%
  #filter(!str_detect(rownames(.), 'Unclassified')) %>%
...
#  filter(Genus %in% genus.text.meta) %>%
#  select(Kingdom:Genus) %>%
#  distinct() %>%
#  mutate(Species = "",
#         OTU = rownames(.))

#otu.meta <- textmap.metagenome_genus %>%
#  filter(!str_detect(rownames(.), 'Unclassified')) %>%
#  rownames_to_column(var = "Genus") %>%
#  left_join(tax.meta %>% select(Genus, OTU)) %>%
#  select(-Genus) %>%
#  column_to_rownames(var = "OTU")

dat.types.genus.meta <- amp_load(otutable = textmap.metagenome_genus,
                     metadata = metadata.metagenome)

dat.types.genus.meta.rel <- amp_load(otutable = textmap.metagenome_genus,
                     metadata = metadata.metagenome) %>%
  #amp_subset_samples(normalise = TRUE)
...
# Aggregate at genus levle
textmap.metagenome_genus.ra <- dat.types.sub3000.meta.ra %>% 
  amp_heatmap(group_by = "seq_id", 
              #min_abundance = 0.001,
              tax_show = 100000,
              tax_aggregate = "Genus", 
...
# Rename genera
genus.text.meta <- textmap.metagenome_genus.ra %>%
  #filter(!str_detect(rownames(.), 'Unclassified')) %>%
  mutate(total = rowSums(.)) %>%
  filter(!total == 0) %>%
  select(total) %>%
...
# Load new genus ampvis object
#tax.meta <- dat.types.sub3000.meta.ra$tax %>%
#  filter(Genus %in% genus.text.meta) %>%
#  select(Kingdom:Genus) %>%
#  distinct() %>%
#  mutate(Species = "",
...
#otu.meta <- textmap.metagenome_genus.ra %>%
#  filter(!str_detect(rownames(.), 'Unclassified')) %>%
#  rownames_to_column(var = "Genus") %>%
#  left_join(tax.meta %>% select(Genus, OTU)) %>%
#  select(-Genus) %>%
...
dat.types.genus.meta.ra <- amp_load(otutable = textmap.metagenome_genus.ra,
                     metadata = metadata.metagenome)

dat.types.genus.meta.ra_rel <- amp_load(otutable = textmap.metagenome_genus.ra,
                     metadata = metadata.metagenome) %>%
  amp_subset_taxa(tax_vector = c("Unclassified"), normalise = TRUE, remove = TRUE)

dat.types.genus.meta.ra.rel_0.1 <- dat.types.genus.meta.ra_rel %>%
  filter_otus(0.1)
```
...
meta_rarecurve <- dat.types.genus.meta.ra %>%
...
tmp <- t(dat.types.genus.meta.ra.rel_0.1$abund)

bray_distance <- as.matrix(vegan::vegdist(vegan::decostand(tmp, method = "hellinger"))) %>%
    as.data.frame() %>%
    rownames_to_column("seq_id")

metadata <- dat.types.genus.meta.ra.rel_0.1$metadata %>%
...
tab_reads <- data.frame(num_reads = apply(dat.types.genus.meta$abund, 2, sum)) %>%
...
rarefied_alphadiversity <- dat.types.genus.meta.ra %>%
...
names <- tibble(
  ...
  new_names = c("Soil type", "Protocol", "Libraries", "Library conc. [ng/µL]", "Total Reads [Gbp]", "Quality Trimmed Reads [Gbp]", "16S Reads Count", "Observed Genera", "Shannon Diversity", "Bray-Curtis Dissimilarity", "BC Dissimilarity\nBetween Protocols"))
...
writexl::write_xlsx(final_table, here("Scaledown", "figures", "S4_table.xlsx"))
```
...
textmap.metagenome_genus <- dat.types.genus.meta %>%
...
ampvis.sub_genus_rel <- amp_load(otutable = textmap.metagenome_genus,
                             metadata = lab.metadata.metagenome) %>%
  amp_subset_taxa(tax_vector = c("Unclassified"), normalise = TRUE, remove = TRUE)
...
heatmap.genus_data <- textmap.sub_genus %>%
  ...
  mutate(label = case_when(#lib_volume == "Small-scale" & !is.na(Bias) ~ "*",
                           lib_volume == "Full-scale" & Bias == "More abundant with 1 x 50 µL" ~ "*",
                           lib_volume == "Small-scale" & Bias == "More abundant with 1 x 5 µL" ~ "*",
                           TRUE ~ ""))
...
for (i in 1:length(soil)) {
  ...
  community_0.1 <- ampvis.sub_genus_rel %>%
    #amp_subset_samples(normalise = TRUE) %>%
    filter_otus(0.1) %>%
  ...
  meta <- dat.types.genus.meta[["metadata"]] %>%
...
```{r ordination}
plot.ordinate <- dat.types.genus.meta %>%
  ...
  scale_shape_manual(values = c(16,21), labels = c("1 x 50 µL", "1 x 5 µL")) +
  labs(title = "All samples: PCA of microbial diversity",
       fill = "Soil type",
       shape = "Library volume",
       tag = "A",
       subtitle = paste0("**PERMANOVA:**", 
                         "<br>Library volume: Variance explained = ","0.3 %,", " p-value = 0.21",
                         "<br>Soil type: Variance explained = ","94.7 %,", " p-value < 0.001",
                         "<br>Interaction: Variance explained = ","0.9 %,", " p-value = 0.42")) +
...
# Per soil type
plot_ord1 <- dat.types.genus.meta %>%
...
plot_ord2 <- dat.types.genus.meta %>%
...
plot_ord3 <- dat.types.genus.meta %>%
...
plot_ord4 <- dat.types.genus.meta %>%
...
plot_ord5 <- dat.types.genus.meta %>%
...
ggsave(here("Scaledown", "figures", "pre_S4Fig.png"), plot = plot.ord, device = "png", height = 12, width = 19)
ggsave(here("Scaledown", "figures", "pre_S4Fig.tif"), plot = plot.ord, device = "tiff", height = 12, width = 19, dpi = 600)
```
...
deseq_results_dataframe <- rlist::list.rbind(deseq_results_genus$deseq) %>%
  mutate(pval_signif = if_else(padj < 0.05, TRUE, FALSE),
         fc_signif = if_else(abs(log2FoldChange) > 1, TRUE, FALSE),)

dat_types_filtered <- ampvis.sub_genus_rel

otu <- dat_types_filtered_genus[["abund"]]

ProtocolDifferentialAbundance <- combine_otu_w_metadata(metadata = dat.types.genus.meta.rel$metadata, otu = otu) %>%
  summarise_triplicates() %>%
  #calculate_relative_abundance() %>%
  filter_double_zeros_meta() %>%
...
plot.diff <- ggplot() +
  ...
  articlethemex0 +
  labs(title = "All samples: Differential abundance of Genera",
...
ggsave(here("Scaledown", "figures", "pre_S6Fig.png"), plot = p.diff, device = "png", height = 12, width = 19)
ggsave(here("Scaledown", "figures", "pre_S6Fig.tif"), plot = p.diff, device = "tiff", height = 12, width = 19, dpi = 600)
```
...
ggsave(here("Scaledown", "figures", "pre_Fig4.png"), plot = gg_arranged, device = "png", height = 12, width = 20)
ggsave(here("Scaledown", "figures", "pre_Fig4.tif"), plot = gg_arranged, device = "tiff", height = 12, width = 20, dpi = 600)
```
