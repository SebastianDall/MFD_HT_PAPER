...
```{r}
dat_types_filtered_0.1 <- filter_otus(dat_types_sub4000_ra, 0.1)
...
metadata_otu_combined_summarized_trip_wide_protocol <- metadata_otu_combined_summarized_trip %>%
  ...
  dplyr::rename(Downscaled = `2 x 5 µL`, Fullscale = `2 x 25 µL`) %>%
  rowwise() %>%
...
```
```{r}
plotDiffRelabund(metadata_otu_combined_summarized_trip_wide_protocol, "Beach Sand")
```
