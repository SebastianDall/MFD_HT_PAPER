...
mean_sd_table <- combine_alpha_beta_with_metadata %>%
  pivot_longer(cols = -c(Soil_type, Protocol, failed_lib, contains("sd")), names_to = "mean", values_to = "mean_value", names_prefix = "mean_") %>%
  pivot_longer(cols = -c(Soil_type, Protocol, failed_lib, contains("mean")), names_to = "sd", values_to = "sd_value", names_prefix = "sd_") %>%
  group_by(Soil_type, Protocol) %>%
...
hjustification_matrix <- as.vector(matrix(c(0, 0, 1, 1, 1, 1), ncol = ncol(mean_sd_table), nrow = nrow(mean_sd_table), byrow = TRUE))
xjustification_matrix <- as.vector(matrix(c(0.1, 0.1, 0.9, 0.9, 0.9, 0.9), ncol = ncol(mean_sd_table), nrow = nrow(mean_sd_table), byrow = TRUE))
...
ggarrange(gg_table_dummy, heatmap, ncol = 2, labels = c("A", "B"), widths = c(1, 1))

ggsave(here("Amplicon_scaledown", "figures", "heatmap_alpha_div_v2.png"), device = "png", width = 14, height = 7)
```
